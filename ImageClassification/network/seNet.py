...
class SeNet(object):
	"""docstring for SeNet"""
	def __init__(self, block_num,is_training,keep_prob,num_classes=100):
	...
	def conv2d(self,inputs,out_channel,kernel_size,stride,relu=True):
		...
		#return tensorlayer.activation.leaky_relu(out, alpha=0.2, name='leaky_relu') if relu else out
	...
	def se_fc_layer(self,inputs):
		...
		#out = tf.layers.dense(out,units=(c//self.ratio),activation=tensorlayer.activation.leaky_relu,kernel_initializer=self.initializer,name='fc_'+str(self.fc_num))
		self.fc_num+=1
	...
	def SE_bottleneck_layer(self,inputs,out_channel,stride):
		...
		#return tensorlayer.activation.leaky_relu(tf.add(out,residual), alpha=0.2, name='leaky_relu')
