...
class Resnet(object):
	...
	def residual_block(self,inputs,output_channel,stride=[1,1]):
		...
		#return tensorlayer.activation.leaky_relu(tf.add(inputs,residual), alpha=0.2, name='leaky_relu')
	...
	def conv2d(self,inputs,output_channel,kernel_size=3,stride=1,relu=True):
		...
		#inputs = tensorlayer.activation.leaky_relu(inputs, alpha=0.2, name='leaky_relu') if relu else inputs
		return inputs
