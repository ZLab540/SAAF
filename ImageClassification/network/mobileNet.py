...
class MobileNet(object):
	...
	def prelu(self, x, scope=None):
		with tf.variable_scope(name_or_scope=scope,default_name='prelu'):
			_alpha = tf.get_variable('prelu',shape=x.get_shape()[-1],dtype=x.dtype,initializer=tf.constant_initializer(0.1))
			pos = tf.nn.relu(x)
			neg = _alpha * (x - tf.abs(x)) * 0.5
		return pos + neg

	def lrelu(self, x, scope=None):
		with tf.variable_scope(name_or_scope=scope,default_name='lrelu'):
			#_alpha = tf.get_variable('prelu',shape=x.get_shape()[-1],dtype=x.dtype,initializer=tf.constant_initializer(0.1))
			pos = tf.nn.relu(x)
			neg = 0.2 * (x - tf.abs(x)) * 0.5
		return pos + neg
