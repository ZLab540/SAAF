...
parser.add_argument('--act', type=str, choices=ACTIVATION_FNS.keys(), default='relu')  # 注释掉
parser.add_argument('--nblocks', type=int, default=100)
parser.add_argument('--brute-force', type=eval, choices=[True, False], default=True)  ### 
...
parser.add_argument('--save', type=str, default='./output/iresnet_toy')
...
class SRS(torch.nn.Module):
    def __init__(self,out_channel,inplace=False):
        super(SRS,self).__init__()
        self.out_channel=out_channel
        self.beta=torch.nn.Parameter(torch.FloatTensor(self.out_channel),requires_grad=True)
        self.delta=torch.nn.Parameter(torch.FloatTensor(self.out_channel),requires_grad=True)
        self.cond1=torch.Tensor(self.out_channel)
        self.beta.data.fill_(2.0)
        self.delta.data.fill_(3.0)
        self.cond1.data.fill_(1.0)

    def forward(self,x):
        _beta=torch.max(self.beta.data.cuda(),self.cond1.data.cuda())
        _delta=torch.max(self.delta.data.cuda(),self.cond1.data.cuda())
        _beta=torch.unsqueeze(_beta,0)
        _delta=torch.unsqueeze(_delta,0)
        if x.dim() == 4:
        	_beta=torch.unsqueeze(_beta,-1)
        	_beta=torch.unsqueeze(_beta,-1)
        	_delta=torch.unsqueeze(_delta,-1)
        	_delta=torch.unsqueeze(_delta,-1)
        return x/(torch.exp(-x/_beta)+x/_delta)
...
if __name__ == '__main__':
    ...
    #activation_fn = ACTIVATION_FNS[args.act] # 注释掉
